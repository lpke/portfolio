...
- Shared content, site-wide data, and reusable configuration that is used across modules should go into `src/app/_utils/constants.ts` when it improves clarity or reuse.
- Component-local constants, implementation details, and one-off values should stay near the component that owns them.
